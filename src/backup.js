...
function BottomScreen({ isVisible, model, onClose }) {
  ...
  useEffect(() => {
    if (isMounted && model && model.glb) {
      ...
      loader.load(model.glb, (gltf) => {
        ...
        camera.position.set(0, 260, 430 );  // Move the camera back to fit the model
  ...
  return (
    <div className={`bottom-screen ${isVisible ? 'visible' : ''}`}>
      ...
      <div className={`bottom-screen-content ${isVisible ? 'visible' : ''}`}>
        ...
        {/* Action Button */}
        {model && <button className="action-btn">Add to Cart</button>}
      </div>
    </div>
  );
}
